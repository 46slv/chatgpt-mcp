...
test("ChatGPT delivery ambiguity retains the conversation slot for reconciliation", async () => {
  ...
  assert.ok(o.conversationArbitrator.inspect(f.chat.conversation_id));
});
test("Codex delivery ambiguity retains the conversation slot for reconciliation", async () => {
...
test("uncancelled timed-out transport excludes another task even after late completion", async () => {
  const conversationStateDir = newRoot("timeout-ownership");
  const a = fixture({ label: "a", conversation: "timeout-shared", conversationStateDir });
  const b = fixture({ label: "b", task_id: "task-b", conversation: "timeout-shared", thread_id: THREAD_B, conversationStateDir });
  let finish;
  let calls = 0;
  const chat = async (input) => { calls++; await new Promise(resolve => { finish = resolve; }); return envelopeFromPayload(input.payload); };
  const oa = orchestrator(a, relayAdapters({ chat, chatgptTimeoutMs: 20 }));
  const ob = orchestrator(b, relayAdapters({ chat }));
  assert.equal((await oa.run()).status, FULL_RELAY_STATES.DELIVERY_UNKNOWN);
  assert.equal((await ob.run()).status, FULL_RELAY_STATES.WAITING_FOR_CONVERSATION_SLOT);
  assert.equal(calls, 1);
  finish();
  await new Promise(resolve => setImmediate(resolve));
  const restarted = orchestrator(a, relayAdapters({ chat }));
  assert.equal((await restarted.run()).status, FULL_RELAY_STATES.DELIVERY_UNKNOWN);
  assert.equal((await ob.run()).status, FULL_RELAY_STATES.WAITING_FOR_CONVERSATION_SLOT);
  assert.equal(calls, 1);
});
