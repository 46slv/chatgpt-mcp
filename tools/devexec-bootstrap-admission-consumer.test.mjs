...
import { validateCodexCompletionEvidence } from "./devexec-closed-loop.mjs";
  ...
  const resultCore = {
    ...
    identities: {
      ...
      initial_completed_turn: {
        ...
        source_turn_sha256: validateCodexCompletionEvidence({ thread_id: THREAD_ID, turn_id: TURN_ID, turn_status: "completed", message: MESSAGE }, THREAD_ID).source_turn_sha256,
...
test("mismatched proof text is refused before persistence and on replay", async () => {
  const { root, exePath, admissionRoot, probes } = setup();
  try {
    const consumer = createBootstrapAdmissionConsumer(validators);
    const { plan, result, candidate } = makeTriple({ workdir: root, exePath });
    const input = { plan, bootstrap_result: result, candidate, execution: { admission_root: admissionRoot, ...probes() } };
    const wrong = { ...input, execution: { ...input.execution, thread_proof: { message: "Unrelated completed response." } } };
    await assert.rejects(() => consumer.admitBootstrapAdmission(wrong), { code: "BOOTSTRAP_CONSUMER_LINKAGE_INVALID" });
    assert.equal(fs.existsSync(admissionRoot), false);
    const admitted = await consumer.admitBootstrapAdmission(input);
    const bytes = fs.readFileSync(admitted.file);
    await assert.rejects(() => consumer.admitBootstrapAdmission(wrong), { code: "BOOTSTRAP_CONSUMER_LINKAGE_INVALID" });
    assert.deepEqual(fs.readFileSync(admitted.file), bytes);
