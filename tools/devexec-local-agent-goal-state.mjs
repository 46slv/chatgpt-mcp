...
function validateAgentRunId(id){if(typeof id!=="string"||!/^LA-[A-Za-z0-9_-]+$/.test(id)||id.includes("..")||path.isAbsolute(id))throw new Error("Invalid local-agent run id");return id;}
export function inspectLocalAgentGoalCompletion({runDir,base,runId}){
 ...
 const agentFile=path.join(base,"ChatGPTMCPProbe","local-agent-runs",validateAgentRunId(owner.agent_run_id)+".json");
