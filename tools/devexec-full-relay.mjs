...
export function createFullRelayOrchestrator({
  ...
  const sendChatGPT = async () => {
    ...
    try {
      ...
      // The timed-out transport may still be active. Keep the exact lease
      // until source-owned reconciliation proves the pending delivery.
  ...
  const injectCodex = async () => {
    ...
    try {
      ...
      // An unknown queue result cannot prove that the pending side effect ended.
      if (state.phase === FULL_RELAY_STATES.REJECTED) releaseLease();
