...
export function createBootstrapAdmissionConsumer({ validate_plan, validate_result, validate_candidate } = {}) {
  ...
  async function admitBootstrapAdmission({ plan, bootstrap_result, candidate, execution = {} } = {}) {
    ...
    const expectedTurnHash = checkedResult.identities.initial_completed_turn.source_turn_sha256;
    if (!SHA256_RE.test(expectedTurnHash ?? "")) {
      fail(BOOTSTRAP_CONSUMER_ERRORS.LINKAGE_INVALID, "Validated bootstrap completion hash is required.");
    }
    ...
    if (threadProof !== null) {
      ...
      const normalized = validateCodexCompletionEvidence({ thread_id: threadId, turn_id: turnId, turn_status: "completed", message }, threadId);
      if (normalized.source_turn_sha256 !== expectedTurnHash) {
        fail(BOOTSTRAP_CONSUMER_ERRORS.LINKAGE_INVALID, "Supplied proof bytes do not match the validated bootstrap completion.");
      }
      threadProbe = async () => ({
        thread_id: threadId,
        turn_id: turnId,
    ...
    const facadeInput = {
      ...
      expected_source_turn_sha256: expectedTurnHash,
    ...
    if (reloaded.codex_continuation_binding.thread_id !== checked.thread_id
      || reloaded.thread_probe.source_turn_sha256 !== expectedTurnHash
