...
function repairPath(){if(!process.env.DEV_EXEC_RUN_ID)return null;return path.join(BASE,"ChatGPTMCPProbe","dev-exec-runs",process.env.DEV_EXEC_RUN_ID,"local-worker-repair.json");}
function statePath(id){if(!/^LW-[A-Za-z0-9-]+$/.test(id))throw new Error("invalid run_id");return path.join(STATE_DIR,`${id}.json`);}
