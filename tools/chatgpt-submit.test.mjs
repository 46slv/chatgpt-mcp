...
test("baseline captures exact url, turn count, and last turn testid", async () => {
  ...
  assert.equal(baseline.maxSeq, 2);
});
...
test("matching composer never retypes", async () => {
  const { page } = submitFake({});
  let typeCalls = 0;
  await ensureExactComposer(page, PROMPT, async () => { typeCalls += 1; return true; });
  assert.equal(typeCalls, 0);
});

test("dropped composer content is re-entered exactly once then verified", async () => {
  const { page, state } = submitFake({ composer: "END DEV EXEC REPORT" });
  let typeCalls = 0;
  const typeFn = async () => { typeCalls += 1; state.composer = PROMPT; return true; };
  await ensureExactComposer(page, PROMPT, typeFn);
  assert.equal(typeCalls, 1);
  assert.equal(state.composer, PROMPT);
});

test("persistent mismatch after one re-entry fails closed without sending", async () => {
  const { page, state } = submitFake({ composer: "END DEV EXEC REPORT" });
  let typeCalls = 0;
  const typeFn = async () => { typeCalls += 1; return true; };
  await assert.rejects(ensureExactComposer(page, PROMPT, typeFn), /send was not attempted/);
  assert.equal(typeCalls, 1);
  assert.equal(state.enterPresses, 0);
});

test("turn seq parsing", () => {
  assert.equal(turnSeqFromTestId("conversation-turn-52"), 52);
  assert.equal(turnSeqFromTestId(null), null);
  assert.equal(turnSeqFromTestId("nope"), null);
});
...
test("ack survives a virtualized window slide (positions shift, seqs grow)", async () => {
  ...
  const page = {
    ...
    keyboard: { press: async () => { st.enterPresses += 1; send(); } },
  };
  const baseline = await captureSendBaseline(page);
  assert.equal(baseline.turnCount, 3);
  assert.equal(baseline.maxSeq, 50);
  const ack = await submitComposedPrompt(page, PROMPT, baseline, FAST);
  assert.equal(ack.userTurnSeq, 51);
  assert.equal(ack.userTurnIndex, 1);
  assert.equal(ack.userTurnText, PROMPT);
  assert.equal(st.enterPresses, 0);
});

test("poll anchor uses global seqs across a window slide", async () => {
  const { observeFreshTurns: observe } = await import("../dist/chatgpt.js");
  const turns = [
    domTurn("conversation-turn-50", "assistant", "old answer", true),
    domTurn("conversation-turn-51", "user", PROMPT, false),
    domTurn("conversation-turn-52", "assistant", "NEW-ANSWER", true),
  ];
  const previous = globalThis.document;
  globalThis.document = { querySelectorAll: (sel) => (sel.indexOf("assistant") >= 0 ? [] : turns) };
  try {
    const page = { evaluate: async (fn, arg) => fn(arg) };
    const snapshot = await observe(page, 51);
    assert.equal(snapshot.turnCount, 3);
    assert.deepEqual(snapshot.freshAssistantTexts, ["NEW-ANSWER"]);
    assert.equal(snapshot.lastFreshHasCopy, true);
  } finally {
    if (previous === undefined) delete globalThis.document;
    else globalThis.document = previous;
...
test("posted chrome stripping removes collapsed-turn expander labels", () => {
  assert.equal(stripPostedChrome("hello"), "hello");
  assert.equal(stripPostedChrome("a" + PROMPT + " \u8868\u793a\u3092\u5897\u3084\u3059"), "a" + PROMPT + " ");
  assert.equal(stripPostedChrome(PROMPT + "Show more"), PROMPT);
  assert.equal(stripPostedChrome(null), "");
});

test("ack accepts the exact prompt with a collapsed-turn expander label", async () => {
  const { page, state } = submitFake({ clickSends: true, sentTurnText: PROMPT + " \u8868\u793a\u3092\u5897\u3084\u3059" });
  const baseline = await captureSendBaseline(page);
  const ack = await submitComposedPrompt(page, PROMPT, baseline, FAST);
  assert.equal(ack.userTurnText, PROMPT);
  assert.equal(state.enterPresses, 0);
