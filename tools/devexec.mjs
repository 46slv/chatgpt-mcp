...
function usage() {
 ...
 " devexec runtime select [--runtime <default|cloud|local>] [--provider <existing|chatgpt|lmstudio|freetoken|llamacpp>] [--enabled|--disabled]",
 " devexec runtime run --task <TaskContract.json> [--runtime local --provider <freetoken|llamacpp>] [--enabled|--disabled] [--evidence <path>] [--output <path>]",
 " devexec runtime metrics summarize <ledger-dir>",
 " devexec runtime recovery scan --state-dir <state-dir>",
 " devexec closed-loop admit --mission-id <id> --task-id <id> --thread-id <uuid> --initial-turn-id <uuid> (--chat-url <url> | --auto-chat) --runtime-path <path> --working-directory <path> [--until-complete]",
