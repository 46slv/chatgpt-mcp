...
- `tools/devexec-mission-launch.mjs`: persists `constraints` in launch state, includes them in semantic idempotency comparison, and exports them to child environment. Missing legacy `constraints` and `target_alias` fields normalize to `[]` / `null` during semantic replay comparison so pre-schema launch records do not spuriously conflict. The child spec always writes `DEV_EXEC_MISSION_CONSTRAINTS_JSON`, including an explicit `[]`, so a constrained parent's ambient process environment cannot leak stale constraints into a later unconstrained child through the launcher's `{...spawn_env, ...spec.env}` merge.
...
- `tools/devexec-mission-constraint-continuation.test.mjs`
  ...
  - an unconstrained child explicitly writes `DEV_EXEC_MISSION_CONSTRAINTS_JSON=[]`, overriding an inherited constrained parent environment;
...
Focused semantic probes also confirmed:

- legacy launch default normalization (`missing constraints -> []`, `missing target_alias -> null`) compares equal to the equivalent current request;
- an unconstrained child spec overwrites a parent `DEV_EXEC_MISSION_CONSTRAINTS_JSON=[...]` with `[]` after environment merge: `INHERITED_CONSTRAINT_ENV_CLEAR=PASS`.
...
3. Complete the constrained child and launch an unrelated second work item with no constraints. Prove the second child gets an explicit `DEV_EXEC_MISSION_CONSTRAINTS_JSON=[]` and the first work's scoped constraints do not leak through inherited process environment.
4. Kill/restart at the existing reliability points: objective write, PENDING launch, LAUNCHING/spawn-before-receipt, LAUNCHED-before-child-attach. No duplicate child, lost constraint envelope, stale constraint inheritance, or target drift.
