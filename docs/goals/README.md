...
The three primary Goals are deliberately separated:
...
None of them owns all three concerns. In particular:
...
1. define shared Event, MissionSnapshot, EpisodeRecord, and MissionResult schemas;
2. implement `DEV-LER-001` fresh Episode mechanics and durable state transitions;
3. expose the read-only `DEV-OBS-001` Control Service and loopback Observatory early enough to verify context rotation and state truth;
4. implement `DEV-EVT-001` new-Mission Task/Consultation admission and exact existing-Mission follow-up;
5. add pause/resume/cancel request Events and later interactive Observatory controls only through the typed ingress;
6. connect additional model/harness adapters and stronger orchestration without changing the Request/Mission/MissionResult contract.
...
- A UI action does not directly mutate state or launch a process.
