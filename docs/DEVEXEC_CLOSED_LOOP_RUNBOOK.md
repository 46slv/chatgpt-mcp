...
returned `devexec.codex-prompt` bytes to the bound thread. `STOP` and
`NEEDS_HUMAN` terminate without queueing. In-flight or ambiguous delivery is
terminal and is never automatically resent.
...
The first-class admission facade and small real development-task canary are
now in place. Future work must preserve the same bounded and exact-identity
invariants:
