...
The first-class `devexec closed-loop ...` CLI/facade now admits an existing
persisted Codex task/thread and exposes bounded `admit`, `run`, and `inspect`
operations. Admission requires exact task/thread/turn identity, the canonical
ChatGPT URL, an absolute native runtime, and an absolute worktree. It does not
route through the older mutable-target fallback path.
